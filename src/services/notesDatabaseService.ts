      ...
      await db.transaction(
        ...
        async () => {
          ...
          if (state.notes.length > 0) {
            const ns = toRaw(state.notes);
            await db.notes.bulkAdd(ns);
          }

          // 保存分类数据
          if (state.categories.length > 0) {
            const cs = toRaw(state.categories);
            await db.categories.bulkAdd(cs);
          }

          // 保存标签数据
          if (state.tags.length > 0) {
            const tagRecords = state.tags.map((name, index) => ({
              id: index.toString(),
              name,
            }));
            const ts = toRaw(tagRecords);
            await db.tags.bulkAdd(ts);
          }
        }
      );
